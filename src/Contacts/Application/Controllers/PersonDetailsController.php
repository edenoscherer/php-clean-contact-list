...
use Edeno\PhpCleanContactList\Contacts\Domain\UseCase\PersonDetails\PersonDetailsUseCase;

/**
 * @author Edeno Luiz Scherer <edenoshcerer@gmail.com>
 */
final class PersonDetailsController
{
    protected PersonDetailsUseCase $personDetailsCase;
    protected JsonPresentation $jsonPresentation;

    public function __construct(PersonDetailsUseCase $personDetailsCase, JsonPresentation $jsonPresentation)
    {
        $this->personDetailsCase = $personDetailsCase;
    ...
    public function __invoke(Request $request, Response $response, array $args): Response
    {
        ...
        try {
            $output = [
                'success' => true,
                'data' => $this->personDetailsCase->handle(intval($args['id']))
            ];
        } catch (Throwable $th) {
            $output = [
                'success' => false,
